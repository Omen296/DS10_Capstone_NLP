...
table1 <- data.table(c(1:7),
                c("Corpus files merged and randomly assigned to 10 stripes of data to enable processing.  Eventually, 90% of the corpus was used for training the model.",
                  ...
                  "All processing is performed using Data Tables, with final output files dic.RDS (336kb) and ngramTOP.RDS (56mb) deployed with the shiny app."
                )
)
...
ngresults %>% kable(
                ...
                column_spec(c(1:4),border_right=T,border_left=T,width="15em")
```
...
table2 <- data.table(c(1:6),
                c("Scrub phrase using the same Quanteda functions as the N-gram processing i.e. convert to lower case, and remove symbols, punctuation, twitter, numbers and URLs",
                  "Convert phrase to list of dictionary IDs to facilitate N-gram lookup",
