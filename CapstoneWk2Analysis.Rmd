...
This prediction model will likely utilize a pre-processed population of N-grams to perform a lookup based on text entered so far; the last N-1 words would be used to lookup the most frequent (hence, most likely) N-gram to predict the Nth word.  If no match can be found, the process is repeated with the next shortest N-gram and so forth.  
...
The main technical challenge of this project is the volume of data provided in the corpus which will generate a large number of N-grams and will be difficult to process on non-enterprise hardware.  Determining the optimal value of 'N', and the most efficient data structures and algorithm, will follow in next stages of the project.  However this initial analysis yielded some ideas to improve performance, for further exploration:
...
* Reduce the population of N-gram predictions by either reducing N, or discarding the population of low frequency (less likely) N-grams which grows larger as N increases.
* Representation of N-grams as lookups/hash tables to reduce redundancy in the data tables and improve performance, both during pre-processing and application runtime.

Throughout this project we will make use of the quanteda package which provides fast multi-threaded tokenization of the source data, and use the data.table package to process the N-gram output from quanteda.

Code executed in this report is not shown, but is available at
...
See Appendix for wordclouds, and histograms of top features and N-grams.  Following is a plot of the CDF showing how many unique words are required to cover 50% and 90% of the corpus, both with and without stopwords:
...
g1 <- ggplot(txsub1comb,aes(x=rank,y=cdf,colour=population)) +
        ...
        geom_vline(xintercept=cum50,linetype="dotted",color="red") +
        geom_vline(xintercept=cum50_nostop,linetype="dotted") +
        geom_text(aes(x=cum50, label=paste0(cum50," words"), y=1), color="red", nudge_x = 2500) +
        geom_hline(yintercept=0.9,linetype="dotted") +
        geom_vline(xintercept=cum90,linetype="dotted") +
        geom_vline(xintercept=cum90_nostop,linetype="dotted",color="red") +
        geom_text(aes(x=cum90, label=paste0(cum90," words"), y=1, hjust="inward"), color="red", nudge_x = 3000) +
        ylim(0,1) +
...
* **Based on this information, one avenue to explore in optimization of the prediction model is to reduce the N-gram model by just focusing on the relatively small subset of word predictions which cover a large proportion of the observed language.**
...
1. Generate all N-grams with N=1:5 (N=1 (unigrams) are needed in the dataset to provide the 'base' for the bi-grams)
...
Note that the number of top ranked N-grams for N=n will (in theory) be the number of unique N-grams for N=(n-1), and for N=1 there is only 1 top-ranked N-gram, which is by definition the most frequent feature in the corpus ("the").  In practice there is a difference of 1 for top-ranked trigrams vs. bigrams and so on.  This analysis ignores beginning and start of lines, so the N-gram sequence generated by quanteda is just a continuous string of the corpus provided.  However on the last line of corpus, if the final bigram is a singular example in the corpus, it will not have a corresponding trigram, and so forth with larger terminal N-grams.  It just so happened in this random sample that the final line was "Seriously, who \<profanity\>" which accounts for the difference of 1 in the counts.  We will ignore this edge condition for now.

This now gives us a basic framework to then decide, given an input of $(w_1...w_{n-1})$ words as starting point, what is the most likely N-gram which extends the sentence to ${w_n}$, for $n\leq5$.

Results of this processing for the 10% sample corpus are shown below.
